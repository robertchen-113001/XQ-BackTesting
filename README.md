...
2. 依需求查閱 `docs/modules/` 下對應功能的 PRD 文件
...
| 文件 | 內容摘要 |
|------|---------|
| [流程架構](./docs/modules/流程架構.md) | 回測流程、逾期處理、重新回測、失敗 Retry、匯出、手動上傳 |
| [回測設定](./docs/modules/回測設定.md) | 系統參數、執行回測對話框（三平台共用與差異模組）、欄位字典 |
| [交易紀錄](./docs/modules/交易紀錄.md) | 資料來源、交易數量規則、統一 CSV 格式、匯入/匯出規格 |
| [匯出功能](./docs/modules/匯出功能.md) | XLSX 完整匯出（11 個分頁規格）|
| [介面設計](./docs/modules/介面設計.md) | UI 架構、置頂區、各分析 Tab（整體統計、交易統計、週期分析、商品與交易）|
| [欄位計算](./docs/modules/欄位計算.md) | 所有欄位的計算公式（含等比模式特殊規格）|
